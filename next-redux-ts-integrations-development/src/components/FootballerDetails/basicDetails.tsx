...
const BasicDetails: React.FC = () => {
  ...
  const footballersData = useSelector(
    (state: any) => state?.footballer?.footballersData
  );
  ...
  const fetchFootballers = async () => {
    await axios
      ...
      .then((res) => {
        ...
        dispatch(getFootballersData(res.data["results"]));
      });
  };
                  ...
                  <Col md={8} className={styles.cardBlock}>
                    <Card className={styles.playerCard}>
                      <Card.Body>
                        ...
                        <Card.Text>Age: {individualPlayerData.age}</Card.Text>
                        <Card.Text>Team: {individualPlayerData.team}</Card.Text>
                        <Button
                          onClick={() => {
                            redirectToUpdatePage(individualPlayerData.id);
                          }}
                        ...
                        <Button
                          onClick={() => {
                            deleteFootballer(individualPlayerData.id);
                          }}
